defmodule MumbleProtocol do
  ...
  defp decode(
         <<type::signed-big-integer-size(16), length::signed-big-integer-size(32), data::binary>>,
         msgs
       )
  ...
  def encode_varint(int) do
    cond do
      ...
      int <= 0xFFFFFFF ->
        <<(int >>> 24 &&& 0xF) ||| 0xE0::8, int >>> 16 &&& 0xFF::8, int >>> 8 &&& 0xFF::8,
          int &&& 0xFF::8>>
  ...
  def encode(struct) do
    ...
    <<type_number::signed-big-integer-size(16), byte_size(binary)::signed-big-integer-size(32)>> <>
      binary
  end
end
